...
**A Markdown editor that gives you the choice — see your `## ` markers, or hide them in a full render. A real [Obsidian](https://obsidian.md) pane.**

marktile (mark + tile) makes Markdown comfortable to read, your way: three modes —
**Seasoned** (headings grow, markers stay put), **Rendered** (full WYSIWYG, markers
hidden), and **Plain** (raw) — switch anytime, same file underneath. Plus smart-Enter
list continuation, **Tab** indentation with a tab-vs-space ruler, and a drag-reorderable
**table of contents**. Just a faster way to edit `.md`.
...
- **Three modes, one file** — Seasoned keeps markers visible while headings grow,
  Rendered hides them for a full WYSIWYG look, Plain shows raw text. Switch anytime;
  the file on disk never changes.
- **Editing comforts** — smart-Enter list continuation, Tab indentation with a ruler,
  and a table of contents you can drag to move whole sections.
...
**マーカーを見せるか、隠すか選べる Markdown エディタ。[Obsidian](https://obsidian.md) の本物のペインとして。**

marktile（mark + tile）は、あなた次第で Markdown を読みやすくします：3 つのモード
——**アジツケ**（見出しは大きく、マーカーはその場に）、**レンダー**（マーカーを隠す
完全 WYSIWYG）、**プレーン**（生テキスト）——いつでも切り替え可能、裏側のファイルは
同じです。さらにスマート Enter でリスト継続、**Tab** インデント（tab とスペースを
見分けるルーラー付き）、ドラッグで並べ替えられる**目次**。`.md` をより速く編集する、
それだけの道具です。
...
- **3 つのモード、1 つのファイル**：アジツケは見出しを大きくしつつマーカーを残し、
  レンダーは完全 WYSIWYG でマーカーを隠し、プレーンは生テキストを表示。いつでも
  切り替え可能、ディスク上のファイルは変わりません。
...
**마커를 보여줄지 숨길지 선택할 수 있는 Markdown 에디터. [Obsidian](https://obsidian.md)의 진짜 패널로.**

marktile(mark + tile)은 당신 방식대로 Markdown을 읽기 좋게 만듭니다: 세 가지 모드
—**양념**(제목은 커지고 마커는 제자리), **렌더**(마커를 숨기는 완전 WYSIWYG),
**담백**(원본 텍스트)—언제든 전환 가능하며, 뒤에 있는 파일은 그대로입니다. 여기에
스마트 Enter로 목록 이어쓰기, **Tab** 들여쓰기(tab과 스페이스를 구분하는 룰러 포함),
드래그로 순서를 바꾸는 **목차**까지. `.md`를 더 빠르게 편집하는, 그 정도의 도구입니다.
...
- **세 가지 모드, 하나의 파일**: 양념은 제목을 키우면서 마커를 남기고, 렌더는 완전
  WYSIWYG로 마커를 숨기고, 담백은 원본 텍스트를 보여줍니다. 언제든 전환 가능하며
  디스크의 파일은 바뀌지 않습니다.
- **편집의 편안함**: 스마트 Enter 목록 이어쓰기, 룰러 포함 Tab 들여쓰기, 목차를
  드래그하면 섹션째 이동.
...
**一個讓你自己選要不要看到標記的 Markdown 編輯器。是 [Obsidian](https://obsidian.md) 的真分頁。**

marktile（mark + tile）讓 Markdown 依你的方式變好讀：三種模式——**調味**（標題放大、
標記留在原地）、**渲染**（完整 WYSIWYG、標記隱藏）、**原味**（純文字）——隨時切換，
底層檔案不變。加上智慧 Enter 接續清單、**Tab** 縮排（附分辨 tab 與空白的標尺），以及
可拖拉重排的**目錄**。就是更快地編輯 `.md`。
...
- **三種模式，同一份檔案**：調味讓標題放大同時留著標記，渲染是完整 WYSIWYG、把標記
  藏起來，原味顯示純文字。隨時切換，磁碟上的檔案永遠不變。
- **編輯的舒適**：智慧 Enter 接續清單、Tab 縮排附標尺、拖拉目錄就能搬動整個章節。
